...
function initDashboardV1Preview() {
  const previewNode = document.querySelector('[data-dashboard-v1-preview="1"]');
  if (!(previewNode instanceof HTMLElement)) {
    return;
  }
    ...
    fetch(endpoint, fetchOptions)
      ...
      .catch((error) => {
        ...
        if (restored) {
          ...
          statusNode.textContent =
            'Nie udało się odświeżyć danych live — pokazuję ostatni zapisany snapshot.';
          debugNode.textContent = `DEBUG: ${String(error?.message || 'REST fetch failed')}`;
          gridNode.hidden = false;
          return;
        }
        statusNode.hidden = false;
...
const initDashboardV1Preview = () => {
  ...
  const fetchPreview = () => {
    ...
    fetch(endpoint, { headers, signal: activeController.signal })
      ...
      if (!hasOperationalData) {
        const counters = payload?.data_health?.counters || {};
        const countersLabel = `trend=${counters.trend_rows || 0}, projekty=${counters.project_rows || 0}, klienci=${counters.client_rows || 0}, kolejka=${counters.queue_rows || 0}, korekty=${counters.adjustment_rows || 0}`;
        statusNode.hidden = false;
        statusNode.textContent =
          payload?.data_health?.hint || 'Brak danych operacyjnych dla wybranego miesiąca.';
        countersNode.textContent = countersLabel;
      ...
      .catch((error) => {
        ...
        try {
          ...
          if (cached) {
            ...
            setSourceState('CACHE', 'cache');
            const counters = payload?.data_health?.counters || {};
            countersNode.textContent = `trend=${counters.trend_rows || 0}, projekty=${counters.project_rows || 0}, klienci=${counters.client_rows || 0}, kolejka=${counters.queue_rows || 0}, korekty=${counters.adjustment_rows || 0}`;
            restored = true;
          }
        } catch (_) {
          restored = false;
  ...
  const fetchPreview = () => {
    ...
    fetch(endpoint, { headers, signal: activeController.signal })
      ...
      if (!hasOperationalData) {
        ...
        statusNode.hidden = false;
        statusNode.textContent =
          `${payload?.data_health?.hint || 'Brak danych operacyjnych dla wybranego miesiąca.'} (${countersLabel})`;
      } else {
        statusNode.hidden = true;
      }
      actionsNode.innerHTML = '';
  ...
  const fetchPreview = () => {
    ...
    fetch(endpoint, { headers, signal: activeController.signal })
      ...
      if (!hasOperationalData) {
        statusNode.hidden = false;
        statusNode.textContent =
          payload?.data_health?.hint ||
          'Brak danych operacyjnych dla wybranego miesiąca.';
      }
  ...
  const fetchPreview = () => {
    ...
    fetch(endpoint, { headers, signal: activeController.signal })
      ...
      .catch((error) => {
        ...
        statusNode.textContent =
          'Nie udało się pobrać podglądu dashboard-v1. Sprawdź uprawnienia i konfigurację REST API.';
        sourceNode.textContent = 'BRAK DANYCH';
        gridNode.hidden = true;
      })
      .finally(() => {
        window.clearTimeout(timeoutHandle);
