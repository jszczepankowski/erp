    ...
    <div class="erp-omd-grid two-columns">
                        ...
                        <td>
                            <select id="time-status" name="status" <?php disabled(! $can_set_status); ?>>
                                <?php foreach (['submitted', 'approved', 'rejected'] as $time_status) : ?>
                                    <option value="<?php echo esc_attr($time_status); ?>" <?php selected($entry['status'] ?? 'submitted', $time_status); ?>><?php echo esc_html($time_status); ?></option>
                                <?php endforeach; ?>
                            </select>
        ...
        <div class="erp-omd-card">
            ...
            <form method="get" class="erp-omd-filter-form">
                ...
                <select name="status"><option value=""><?php esc_html_e('Wszystkie statusy', 'erp-omd'); ?></option><?php foreach (['submitted', 'approved', 'rejected'] as $time_status) : ?><option value="<?php echo esc_attr($time_status); ?>" <?php selected((string) ($filters['status'] ?? ''), $time_status); ?>><?php echo esc_html($time_status); ?></option><?php endforeach; ?></select>
                <button class="button" type="submit"><?php esc_html_e('Filtruj', 'erp-omd'); ?></button>
            </form>
            <table class="widefat striped">
                <thead><tr><th><?php esc_html_e('Data', 'erp-omd'); ?></th><th><?php esc_html_e('Pracownik', 'erp-omd'); ?></th><th><?php esc_html_e('Projekt', 'erp-omd'); ?></th><th><?php esc_html_e('Rola', 'erp-omd'); ?></th><th><?php esc_html_e('Godziny', 'erp-omd'); ?></th><th><?php esc_html_e('Status', 'erp-omd'); ?></th><th><?php esc_html_e('Snapshot', 'erp-omd'); ?></th><th><?php esc_html_e('Akcje', 'erp-omd'); ?></th></tr></thead>
                <tbody>
                                ...
                                <td>
                                    ...
                                    <?php if ($this->time_entry_service->can_approve_entry($time_row, wp_get_current_user()) && $time_row['status'] !== 'approved') : ?>
                                        <form method="post" class="erp-omd-inline-form">
                                            ...
                                            <button class="button button-small" type="submit"><?php esc_html_e('Approve', 'erp-omd'); ?></button>
                                        </form>
                                    <?php endif; ?>
                                    <?php if ($this->time_entry_service->can_approve_entry($time_row, wp_get_current_user()) && $time_row['status'] !== 'rejected') : ?>
                                        <form method="post" class="erp-omd-inline-form">
                                            ...
                                            <button class="button button-small" type="submit"><?php esc_html_e('Reject', 'erp-omd'); ?></button>
                                        </form>
                                    <?php endif; ?>
