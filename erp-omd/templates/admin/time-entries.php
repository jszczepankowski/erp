    ...
    <div class="erp-omd-card">
        ...
        <form method="post">
            ...
            <table class="form-table">
                ...
                <tr><th><label for="time-project"><?php esc_html_e('Projekt', 'erp-omd'); ?></label></th><td><select id="time-project" name="project_id" required><option value=""><?php esc_html_e('Wybierz projekt', 'erp-omd'); ?></option><?php foreach ($projects_for_time as $project_item) : ?><option value="<?php echo esc_attr($project_item['id']); ?>" data-client-id="<?php echo esc_attr($project_item['client_id']); ?>" <?php selected((int) ($entry['project_id'] ?? 0), (int) $project_item['id']); ?>><?php echo esc_html($project_item['name'] . ' [' . $this->project_status_label($project_item['status']) . ']'); ?></option><?php endforeach; ?></select></td></tr>
