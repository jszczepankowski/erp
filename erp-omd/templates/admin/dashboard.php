<div class="wrap erp-omd-admin">
    ...
    $dashboard_metric_tiles = [
        ...
        [
            'icon' => 'chart-line',
            'value' => number_format_i18n((float) ($dashboard_employee_hourly_profit ?? 0), 2),
    ...
    <div class="erp-omd-dashboard-sections" data-dashboard-sections="1">
        <div class="erp-omd-dashboard-top-row">
        ...
        <section class="erp-omd-dashboard-section" data-section-id="private-tasks">
            <div class="erp-omd-section-header">
            ...
            <form method="post" class="erp-omd-form-grid">
                ...
                <p><label for="erp-omd-admin-task-text"><?php esc_html_e('Treść taska', 'erp-omd'); ?></label><br><textarea id="erp-omd-admin-task-text" name="task_text" rows="2" class="large-text" required></textarea></p>
                <p><label for="erp-omd-admin-task-date"><?php esc_html_e('Termin', 'erp-omd'); ?></label><br><input id="erp-omd-admin-task-date" type="date" name="task_due_date" value="<?php echo esc_attr(current_time('Y-m-d')); ?>"></p>
                <p><button type="submit" class="button button-primary"><?php esc_html_e('Dodaj task', 'erp-omd'); ?></button></p>
            </form>
            <p>
                <?php foreach (['all' => __('Wszystkie', 'erp-omd'), 'today' => __('Na dziś', 'erp-omd'), 'incomplete' => __('Niedokończone', 'erp-omd')] as $task_filter_key => $task_filter_label) : ?>
                    <?php $task_filter_url = add_query_arg(['page' => 'erp-omd', 'reporting_month' => $reporting_month, 'tasks_filter' => $task_filter_key], admin_url('admin.php')); ?>
                    <?php if (($dashboard_private_tasks_filter ?? 'all') === $task_filter_key) : ?><strong><?php endif; ?>
                    <a href="<?php echo esc_url($task_filter_url); ?>"><?php echo esc_html($task_filter_label); ?></a><?php if (($dashboard_private_tasks_filter ?? 'all') === $task_filter_key) : ?></strong><?php endif; ?><?php echo $task_filter_key === 'incomplete' ? '' : ' · '; ?>
                <?php endforeach; ?>
            </p>
                    ...
                    <tr>
                        ...
                        <td>
                            <form method="post">
                                ...
                                <button type="submit" class="button button-secondary"><?php echo ! empty($task_row['completed']) ? esc_html__('Zrobione', 'erp-omd') : esc_html__('Niedokończone', 'erp-omd'); ?></button>
                            </form>
                        </td>
                    </tr>
                <?php endforeach; else : ?>
