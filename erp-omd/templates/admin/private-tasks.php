                    ...
                    <?php if (! empty($dashboard_private_tasks)) : foreach ($dashboard_private_tasks as $i => $task_row) : $task_id = (string) ($task_row['task_id'] ?? ''); ?>
                        <tr>
                            ...
                            <td>
                                ...
                                <a class="button button-small" href="<?php echo esc_url(add_query_arg(['page' => 'erp-omd-private-tasks', 'task_action' => 'toggle', 'task_id' => $task_id], admin_url('admin.php'))); ?>"><?php esc_html_e('Zmień status', 'erp-omd'); ?></a>
                                <a class="button button-small button-link-delete" onclick="return confirm('<?php echo esc_js(__('Usunąć zadanie?', 'erp-omd')); ?>');" href="<?php echo esc_url(add_query_arg(['page' => 'erp-omd-private-tasks', 'task_action' => 'delete', 'task_id' => $task_id], admin_url('admin.php'))); ?>"><?php esc_html_e('Usuń', 'erp-omd'); ?></a>
                            </td>
                        </tr>
                    <?php endforeach; else : ?>
