...
class ERP_OMD_KSeF_Import_Service
{
     ...
     * @return array<string,mixed>
     */
    public function attach_sales_document_to_project($sales_id, $project_id, $is_final, $user_id)
    {
        ...
        foreach ($rows as $index => $row) {
            ...
            if ($is_final) {
                ...
                $rows[$index]['final_marked_at'] = '';
                $rows[$index]['final_marked_by_user_id'] = 0;
            }

            $this->save_sales_inbox($rows);
            $this->append_sales_audit((int) $sales_id, (int) $user_id, $before, (array) $rows[$index]);

            return ['ok' => true, 'row' => $rows[$index], 'errors' => []];
        }
    ...
    public function has_final_sales_invoice_for_project($project_id)
    {
        ...
        foreach ($this->load_sales_inbox() as $row) {
            if ((int) ($row['project_id'] ?? 0) !== $project_id) {
                continue;
            }
     ...
     * @return void
     */
    private function append_sales_audit($sales_id, $user_id, array $before, array $after)
    {
        if (! function_exists('get_option') || ! function_exists('update_option')) {
            return;
        }

        $log = (array) get_option(self::OPTION_SALES_AUDIT, []);
        $log[] = [
            'sales_id' => (int) $sales_id,
            'user_id' => (int) $user_id,
            'before' => $before,
        ...
        update_option(self::OPTION_SALES_AUDIT, $log, false);
    }
