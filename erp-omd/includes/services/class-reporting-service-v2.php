...
class ERP_OMD_Reporting_Service
{
    ...
    public function build_project_report(array $filters)
    {
        ...
        foreach ($projects as $project) {
            ...
            $revenue = $project_revenue + $time_revenue_component;
            $cost = $direct_cost + $time_cost_component;
            $profit = $revenue - $cost;
            ...
            case 'invoice':
                ...
                foreach ($rows as $row) {
                    $invoice_rows[] = [
                        ...
                        number_format((float) ($row['retainer_amount'] ?? 0), 2, '.', ''),
                        number_format((float) $row['reported_hours'], 2, '.', ''),
                        $row['entries_count'],
                        number_format((float) $row['filtered_time_revenue'], 2, '.', ''),
                        number_format((float) ($row['additional_revenue'] ?? 0), 2, '.', ''),
                        number_format((float) $row['filtered_time_cost'], 2, '.', ''),
                        number_format((float) $row['filtered_direct_cost'], 2, '.', ''),
                        number_format((float) $row['revenue'], 2, '.', ''),
                        number_format((float) $row['cost'], 2, '.', ''),
                ...
                return [
                    ...
                    'headers' => ['Klient', 'Projekt', 'Typ rozliczenia', 'Manager', 'Budżet', 'Abonament', 'Godziny', 'Wpisy', 'Przychód czasu (filtrowany)', 'Przychód dodatkowy', 'Koszt czasu (filtrowany)', 'Koszt bezpośredni (filtrowany)', 'Przychód łącznie', 'Koszt łącznie', 'Zysk', 'Marża %', 'Status'],
                    'rows' => $invoice_rows,
                ];
            case 'projects':
    ...
    private function get_filtered_entries(array $project_ids, array $filters)
    {
        ...
        return $this->filter_entries_from_pool((array) $entries, $project_ids, $filters);
    }

    private function filter_entries_from_pool(array $entries, array $project_ids, array $filters)
    {
        $report_type = (string) ($filters['report_type'] ?? '');
    ...
    private function get_revenue_metrics_by_project(array $project_ids, $month)
    {
        $metrics = [];
    ...
    private function build_project_revenue_index_by_month(array $projects, array $months)
    {
        ...
        foreach ($months as $month) {
            ...
            $index[(string) $month] = round(array_sum(array_map(static function ($row) {
                return (float) ($row['project_revenue'] ?? 0.0);
            }, $month_rows)), 2);
        }
    ...
    private function build_direct_cost_index_by_month_and_project(array $project_ids, array $months)
    {
        $index = [];
        $allowed_months = array_fill_keys(array_map('strval', $months), true);
        foreach ($months as $month) {
    ...
    private function build_fixed_cost_index_by_month(array $months, array $month_ranges = [])
    {
        ...
        foreach ($months as $month) {
            ...
            $index[$month_key] = $fixed_cost > 0 ? $fixed_cost : $default_fixed_cost;
        }
