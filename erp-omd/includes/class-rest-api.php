...
class ERP_OMD_REST_API
{
    ...
    private function register_supplier_routes()
    {
        ...
        register_rest_route('erp-omd/v1', '/suppliers/(?P<id>\d+)', [
            ...
            ['methods' => WP_REST_Server::DELETABLE, 'callback' => [$this, 'delete_supplier'], 'permission_callback' => [$this, 'can_manage_projects']],
        ]);
        ...
        register_rest_route('erp-omd/v1', '/cost-invoices/(?P<id>\d+)', [
            ...
            ['methods' => WP_REST_Server::DELETABLE, 'callback' => [$this, 'delete_cost_invoice'], 'permission_callback' => [$this, 'can_manage_projects']],
        ]);
        ...
        register_rest_route('erp-omd/v1', '/ksef/sales/(?P<sales_id>\d+)/attach', [
            ['methods' => WP_REST_Server::CREATABLE, 'callback' => [$this, 'attach_ksef_sales_document'], 'permission_callback' => [$this, 'can_manage_projects']],
        ]);
        ...
        register_rest_route('erp-omd/v1', '/ksef/cost/import-xml', [
            ['methods' => WP_REST_Server::CREATABLE, 'callback' => [$this, 'import_ksef_cost_xml'], 'permission_callback' => [$this, 'can_manage_projects']],
        ]);
        register_rest_route('erp-omd/v1', '/ksef/sync/status', [
            ['methods' => WP_REST_Server::READABLE, 'callback' => [$this, 'ksef_sync_status'], 'permission_callback' => [$this, 'can_manage_projects']],
        ]);
        register_rest_route('erp-omd/v1', '/ksef/sync/run', [
            ['methods' => WP_REST_Server::CREATABLE, 'callback' => [$this, 'run_ksef_sync_now'], 'permission_callback' => [$this, 'can_manage_projects']],
        ]);
    }
    ...
    public function delete_supplier(WP_REST_Request $request) { $supplier_id = (int) $request['id']; $supplier = $this->suppliers->find($supplier_id); if (! $supplier) { return new WP_Error('erp_omd_supplier_not_found', __('Supplier not found.', 'erp-omd'), ['status' => 404]); } $invoices = (array) $this->cost_invoices->list(['supplier_id' => $supplier_id]); foreach ($invoices as $invoice) { $invoice_id = (int) ($invoice['id'] ?? 0); if ($invoice_id <= 0) { continue; } $this->cost_invoice_audit->insert_many([['invoice_id' => $invoice_id, 'field_name' => '__deleted__', 'before_value' => wp_json_encode($invoice), 'after_value' => '', 'changed_by_user_id' => get_current_user_id(), 'changed_at' => current_time('mysql')]]); $project_id = (int) ($invoice['project_id'] ?? 0); $description = sprintf('%s #%d (%s)', __('Faktura kosztowa', 'erp-omd'), $invoice_id, (string) ($invoice['invoice_number'] ?? '')); if ($project_id > 0) { $project_cost_rows = (array) $this->project_costs->for_project($project_id); foreach ($project_cost_rows as $project_cost_row) { $project_cost_description = (string) ($project_cost_row['description'] ?? ''); if ($project_cost_description === $description || strpos($project_cost_description, $description . ' / ') === 0) { $this->project_costs->delete((int) ($project_cost_row['id'] ?? 0)); } } $this->project_financial_service->rebuild_for_project($project_id); } $this->cost_invoices->delete($invoice_id); if ($this->cost_invoice_items && method_exists($this->cost_invoice_items, 'delete_for_invoice')) { $this->cost_invoice_items->delete_for_invoice($invoice_id); } } $this->suppliers->delete($supplier_id); return new WP_REST_Response(null, 204); }
    public function list_cost_invoices(WP_REST_Request $request) { $filters = ['supplier_id' => (int) $request->get_param('supplier_id'), 'project_id' => (int) $request->get_param('project_id'), 'status' => sanitize_text_field((string) $request->get_param('status'))]; $rows = (array) $this->cost_invoices->list($filters); foreach ($rows as $index => $row) { $rows[$index] = $this->attach_items_to_cost_invoice((array) $row); } return rest_ensure_response($rows); }
    public function get_cost_invoice(WP_REST_Request $request) { $invoice = $this->cost_invoices->find((int) $request['id']); if (! $invoice) { return new WP_Error('erp_omd_cost_invoice_not_found', __('Cost invoice not found.', 'erp-omd'), ['status' => 404]); } return rest_ensure_response($this->attach_items_to_cost_invoice((array) $invoice)); }
    public function create_cost_invoice(WP_REST_Request $request) { $payload = $this->sanitize_cost_invoice_payload($request); $payload['created_by_user_id'] = get_current_user_id(); $payload['updated_by_user_id'] = get_current_user_id(); $result = $this->cost_invoice_workflow->create_invoice($payload); if (! (bool) ($result['ok'] ?? false)) { return new WP_Error('erp_omd_cost_invoice_invalid', implode(' ', (array) ($result['errors'] ?? [])), ['status' => 422]); } return new WP_REST_Response($this->attach_items_to_cost_invoice((array) $this->cost_invoices->find((int) $result['invoice_id'])), 201); }
    public function update_cost_invoice(WP_REST_Request $request) { $invoice_id = (int) $request['id']; $existing = $this->cost_invoices->find($invoice_id); if (! $existing) { return new WP_Error('erp_omd_cost_invoice_not_found', __('Cost invoice not found.', 'erp-omd'), ['status' => 404]); } $payload = $this->sanitize_cost_invoice_payload($request, $existing); $payload['updated_by_user_id'] = get_current_user_id(); $result = $this->cost_invoice_workflow->update_invoice($invoice_id, $payload, get_current_user_id()); if (! (bool) ($result['ok'] ?? false)) { return new WP_Error('erp_omd_cost_invoice_invalid', implode(' ', (array) ($result['errors'] ?? [])), ['status' => 422]); } return rest_ensure_response($this->attach_items_to_cost_invoice((array) $this->cost_invoices->find($invoice_id))); }
    public function delete_cost_invoice(WP_REST_Request $request) { $invoice_id = (int) $request['id']; $existing = $this->cost_invoices->find($invoice_id); if (! $existing) { return new WP_Error('erp_omd_cost_invoice_not_found', __('Cost invoice not found.', 'erp-omd'), ['status' => 404]); } $this->cost_invoice_audit->insert_many([['invoice_id' => $invoice_id, 'field_name' => '__deleted__', 'before_value' => wp_json_encode($existing), 'after_value' => '', 'changed_by_user_id' => get_current_user_id(), 'changed_at' => current_time('mysql')]]); $this->cost_invoices->delete($invoice_id); if ($this->cost_invoice_items && method_exists($this->cost_invoice_items, 'delete_for_invoice')) { $this->cost_invoice_items->delete_for_invoice($invoice_id); } return new WP_REST_Response(null, 204); }
    ...
    public function attach_ksef_sales_document(WP_REST_Request $request) { $sales_id = (int) $request['sales_id']; $project_id = (int) $request->get_param('project_id'); $is_final = (bool) $request->get_param('is_final'); $result = $this->ksef_import_service->attach_sales_document_to_project($sales_id, $project_id, $is_final, get_current_user_id()); if (! (bool) ($result['ok'] ?? false)) { return new WP_Error('erp_omd_ksef_sales_attach_invalid', implode(' ', (array) ($result['errors'] ?? [])), ['status' => 422]); } return rest_ensure_response((array) ($result['row'] ?? [])); }
    ...
    public function import_ksef_cost_xml(WP_REST_Request $request) { $xml_content = (string) $request->get_param('xml_content'); if (trim($xml_content) === '') { return new WP_Error('erp_omd_ksef_cost_xml_invalid', __('XML content is required.', 'erp-omd'), ['status' => 422]); } return rest_ensure_response($this->ksef_import_service->import_cost_xml($xml_content, get_current_user_id())); }
    public function ksef_sync_status(WP_REST_Request $request) { $environment = strtoupper(sanitize_text_field((string) get_option('erp_omd_ksef_sync_hub_env', 'TEST'))); if (! in_array($environment, ['TEST', 'DEMO', 'PRD'], true)) { $environment = 'TEST'; } $mode = sanitize_key((string) get_option('erp_omd_ksef_sync_hub_mode', 'dry_run')); if (! in_array($mode, ['dry_run', 'active'], true)) { $mode = 'dry_run'; } $subject_types = (array) get_option('erp_omd_ksef_sync_subject_types', ['subject1']); $subject_types = array_values(array_filter(array_map('sanitize_key', $subject_types))); if ($subject_types === []) { $subject_types = ['subject1']; } $state = (array) get_option('erp_omd_ksef_sync_state_' . strtolower($environment), []); $hwm = []; foreach ($subject_types as $subject_type) { $hwm[$subject_type] = (string) get_option('erp_omd_ksef_sync_hwm_' . strtolower($environment) . '_' . sanitize_key((string) $subject_type), ''); } $last_dry_run = (array) get_option('erp_omd_ksef_sync_hub_last_dry_run', []); $auth = class_exists('ERP_OMD_KSeF_Auth_Storage') ? (array) (new ERP_OMD_KSeF_Auth_Storage())->get_tokens($environment) : []; return rest_ensure_response(['enabled' => (bool) get_option('erp_omd_ksef_sync_hub_enabled', false), 'mode' => $mode, 'environment' => $environment, 'base_url' => (string) get_option('erp_omd_ksef_api_base_url', ''), 'subject_types' => $subject_types, 'has_context_identifier' => trim((string) get_option('erp_omd_ksef_sync_hub_context_identifier', '')) !== '', 'has_ap_token' => trim((string) get_option('erp_omd_ksef_sync_hub_ap_token_enc', '')) !== '', 'has_public_key' => trim((string) get_option('erp_omd_ksef_public_key_' . strtolower($environment), '')) !== '', 'sync_state' => $state, 'hwm' => $hwm, 'auth' => ['has_access_token' => ! empty($auth['access_token']), 'access_expires_at' => (string) ($auth['access_expires_at'] ?? ''), 'refresh_expires_at' => (string) ($auth['refresh_expires_at'] ?? '')], 'last_dry_run' => $last_dry_run]); }
    public function run_ksef_sync_now(WP_REST_Request $request) { $environment = strtoupper(sanitize_text_field((string) $request->get_param('environment'))); if (! in_array($environment, ['TEST', 'DEMO', 'PRD'], true)) { $environment = strtoupper(sanitize_text_field((string) get_option('erp_omd_ksef_sync_hub_env', 'TEST'))); } $mode = sanitize_key((string) $request->get_param('mode')); if (! in_array($mode, ['dry_run', 'active'], true)) { $mode = sanitize_key((string) get_option('erp_omd_ksef_sync_hub_mode', 'dry_run')); } if (! in_array($mode, ['dry_run', 'active'], true)) { $mode = 'dry_run'; } $base_url = (string) get_option('erp_omd_ksef_api_base_url', ''); if ($base_url === '') { return new WP_Error('erp_omd_ksef_sync_base_url_missing', __('KSeF API base URL is missing.', 'erp-omd'), ['status' => 422]); } $connector = new ERP_OMD_KSeF_Connector($base_url); $export_service = new ERP_OMD_KSeF_Export_Service($connector); $invoice_repository = new ERP_OMD_Cost_Invoice_Repository(); $audit_repository = new ERP_OMD_Cost_Invoice_Audit_Repository(); $supplier_repository = new ERP_OMD_Supplier_Repository(); $client_repository = new ERP_OMD_Client_Repository(); $workflow = new ERP_OMD_Cost_Invoice_Workflow_Service($invoice_repository, $audit_repository, $supplier_repository, new ERP_OMD_Project_Repository()); $import_service = $mode === 'active' ? new ERP_OMD_KSeF_Import_Service($workflow, $invoice_repository, $audit_repository, null, null, $supplier_repository, $client_repository) : null; $service = new ERP_OMD_KSeF_Incremental_Sync_Service(null, null, $export_service, $import_service); $result = (array) $service->run_scheduled_sync($environment, 3); $result['mode'] = $mode; return rest_ensure_response($result); }
    ...
    private function sanitize_cost_invoice_payload(WP_REST_Request $request, array $existing = null) { $payload = ['supplier_id' => (int) $this->request_param_or_default($request, 'supplier_id', $existing['supplier_id'] ?? 0), 'project_id' => (int) $this->request_param_or_default($request, 'project_id', $existing['project_id'] ?? 0), 'invoice_number' => sanitize_text_field((string) $this->request_param_or_default($request, 'invoice_number', $existing['invoice_number'] ?? '')), 'issue_date' => sanitize_text_field((string) $this->request_param_or_default($request, 'issue_date', $existing['issue_date'] ?? '')), 'status' => sanitize_text_field((string) $this->request_param_or_default($request, 'status', $existing['status'] ?? '')), 'description' => sanitize_textarea_field((string) $this->request_param_or_default($request, 'description', $existing['description'] ?? '')), 'net_amount' => (float) $this->request_param_or_default($request, 'net_amount', $existing['net_amount'] ?? 0), 'vat_amount' => (float) $this->request_param_or_default($request, 'vat_amount', $existing['vat_amount'] ?? 0), 'gross_amount' => (float) $this->request_param_or_default($request, 'gross_amount', $existing['gross_amount'] ?? 0), 'source' => sanitize_text_field((string) $this->request_param_or_default($request, 'source', $existing['source'] ?? 'manual')) ?: 'manual', 'ksef_reference_number' => sanitize_text_field((string) $this->request_param_or_default($request, 'ksef_reference_number', $existing['ksef_reference_number'] ?? ''))]; if ($request->get_param('items') !== null) { $payload['items'] = $this->sanitize_cost_invoice_items($request->get_param('items')); } return $payload; }
    private function sanitize_ksef_document_payload($document) { $document = is_array($document) ? $document : []; return ['supplier_id' => (int) ($document['supplier_id'] ?? 0), 'project_id' => (int) ($document['project_id'] ?? 0), 'invoice_number' => sanitize_text_field((string) ($document['invoice_number'] ?? '')), 'issue_date' => sanitize_text_field((string) ($document['issue_date'] ?? '')), 'net_amount' => (float) ($document['net_amount'] ?? 0), 'vat_amount' => (float) ($document['vat_amount'] ?? 0), 'gross_amount' => (float) ($document['gross_amount'] ?? 0), 'items' => $this->sanitize_cost_invoice_items((array) ($document['items'] ?? [])), 'ksef_reference_number' => sanitize_text_field((string) ($document['ksef_reference_number'] ?? '')), 'buyer_nip' => sanitize_text_field((string) ($document['buyer_nip'] ?? $document['nabywca_nip'] ?? '')), 'seller_nip' => sanitize_text_field((string) ($document['seller_nip'] ?? $document['sprzedawca_nip'] ?? '')), 'our_company_nip' => sanitize_text_field((string) ($document['our_company_nip'] ?? $document['company_nip'] ?? get_option('erp_omd_company_nip', '')))]; }
     ...
     */
    private function sanitize_cost_invoice_items($items_raw)
    {
        ...
        foreach ($items_raw as $index => $item_raw) {
            ...
            $items[] = [
                ...
                'qty' => (float) ($item_raw['qty'] ?? 0),
                'unit' => sanitize_text_field((string) ($item_raw['unit'] ?? '')),
                'unit_net_amount' => (float) ($item_raw['unit_net_amount'] ?? 0),
                'net_amount' => (float) ($item_raw['net_amount'] ?? 0),
                'vat_rate' => (float) ($item_raw['vat_rate'] ?? 0),
                'vat_amount' => (float) ($item_raw['vat_amount'] ?? 0),
                'gross_amount' => (float) ($item_raw['gross_amount'] ?? 0),
                'source_payload' => (array) ($item_raw['source_payload'] ?? []),
            ];
        }
     ...
     */
    private function attach_items_to_cost_invoice(array $invoice)
    {
        $invoice_id = (int) ($invoice['id'] ?? 0);
        if ($invoice_id <= 0 || ! $this->cost_invoice_items || ! method_exists($this->cost_invoice_items, 'for_invoice')) {
            $invoice['items'] = [];
            return $invoice;
        }
    ...
    private function is_project_cost_locked_for_non_admin($project_id)
    {
        ...
        return in_array((string) ($project['status'] ?? ''), ['zakonczony', 'archiwum'], true);
    }
