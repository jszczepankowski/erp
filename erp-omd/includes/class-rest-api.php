...
class ERP_OMD_REST_API
{
    ...
    public function register_routes()
    {
        ...
        register_rest_route('erp-omd/v1', '/periods', [
            ['methods' => WP_REST_Server::READABLE, 'callback' => [$this, 'periods_index_endpoint_v1'], 'permission_callback' => [$this, 'can_access_reports']],
        ]);
        register_rest_route('erp-omd/v1', '/periods/(?P<month>\\d{4}-\\d{2})', [
            ['methods' => WP_REST_Server::READABLE, 'callback' => [$this, 'period_status_endpoint'], 'permission_callback' => [$this, 'can_access_reports']],
        ]);
        register_rest_route('erp-omd/v1', '/periods/(?P<month>\\d{4}-\\d{2})/transition', [
            ['methods' => WP_REST_Server::CREATABLE, 'callback' => [$this, 'period_transition_endpoint_v1'], 'permission_callback' => [$this, 'can_manage_settings']],
        ]);
        register_rest_route('erp-omd/v1', '/adjustments', [
            ['methods' => WP_REST_Server::READABLE, 'callback' => [$this, 'adjustments_index_endpoint_v1'], 'permission_callback' => [$this, 'can_manage_settings']],
            ['methods' => WP_REST_Server::CREATABLE, 'callback' => [$this, 'adjustments_create_endpoint_v1'], 'permission_callback' => [$this, 'can_manage_settings']],
        ]);
    ...
    public function period_status_endpoint(WP_REST_Request $request)
    {
    ...
    public function periods_index_endpoint_v1()
    {
        return rest_ensure_response($this->period_service->list_periods());
    }

    public function period_transition_endpoint_v1(WP_REST_Request $request)
    {
        $month = sanitize_text_field((string) $request['month']);
    ...
    public function adjustments_index_endpoint_v1(WP_REST_Request $request)
    {
        $filters = [
    ...
    public function adjustments_create_endpoint_v1(WP_REST_Request $request)
    {
        $month = sanitize_text_field((string) $request->get_param('month'));
