...
class ERP_OMD_Frontend
{
    ...
    public function handle_front_request()
    {
        ...
        if ($screen === 'estimate-decision') {
            $this->handle_public_estimate_decision_screen();
            return;
        }
    ...
    private function handle_public_estimate_decision_screen()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['erp_omd_front_action']) && wp_unslash((string) $_POST['erp_omd_front_action']) === 'estimate_decision') {
            ...
            $state = $this->resolve_public_estimate_decision_state($token);
            if ($state instanceof WP_Error) {
                ...
                if ($decision === 'accept') {
                    ...
                    if ($result instanceof WP_Error) {
                        ...
                        $decision_done = true;
                        $this->invalidate_public_estimate_decision_token($estimate_id);
                    }
                } else {
        ...
        if ($state instanceof WP_Error) {
            ...
            $estimate_items = $estimate_id > 0 ? $this->estimate_items->for_estimate($estimate_id) : [];
    ...
    private function resolve_public_estimate_decision_state($token)
    {
        $token = (string) $token;
        if ($token === '') {
    ...
    private function invalidate_public_estimate_decision_token($estimate_id)
    {
    ...
    private function handle_public_estimate_decision_screen()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['erp_omd_front_action']) && wp_unslash((string) $_POST['erp_omd_front_action']) === 'estimate_decision') {
            ...
            $selected_decision = in_array($decision, ['accept', 'reject'], true) ? $decision : 'accept';
            $comment_value = $comment;
            $state = $this->resolve_estimate_decision_state($token);
            if ($state instanceof WP_Error) {
                $notice_type = 'error';
                $notice_message = $state->get_error_message();
            } elseif (! in_array($decision, ['accept', 'reject'], true)) {
                ...
                if ($decision === 'accept') {
                    ...
                    if ($result instanceof WP_Error) {
                        ...
                        $notice_message = __('Dziękujemy. Kosztorys został zaakceptowany.', 'erp-omd');
                        $decision_done = true;
                        $this->invalidate_estimate_decision_token($estimate_id);
                    }
                } else {
                    $estimate = (array) $state['estimate'];
                    $estimate['status'] = 'odrzucony';
    ...
    private function resolve_estimate_decision_state($token)
    {
        $token = (string) $token;
        if ($token === '') {
            return new WP_Error('erp_omd_estimate_token_missing', __('Brak tokenu decyzji kosztorysu.', 'erp-omd'));
    ...
    private function invalidate_estimate_decision_token($estimate_id)
    {
        $estimate_id = (int) $estimate_id;
        if ($estimate_id <= 0) {
            return;
    ...
    private function handle_estimate_decision_screen()
    {
        $token = sanitize_text_field((string) ($_GET['token'] ?? $_POST['token'] ?? ''));
        $notice_type = '';
        $notice_message = '';
