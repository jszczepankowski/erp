...
if (! class_exists('ERP_OMD_Cost_Invoice_Workflow_Service')) {
    class ERP_OMD_Cost_Invoice_Workflow_Service
    {
        ...
        public $audit_repository;
        public $supplier_repository;
...
$module = $container->ksef_module();

if ($module !== $container->ksef_module()) {
    throw new RuntimeException('Container should cache the KSeF module instance.');
}
if ($module->supplier_repository() !== $module->supplier_repository()) {
    throw new RuntimeException('KSeF module should cache supplier repository instances.');
}
if ($module->cost_invoice_repository() !== $module->cost_invoice_repository()) {
    throw new RuntimeException('KSeF module should cache cost invoice repository instances.');
}
if ($module->cost_invoice_item_repository() !== $module->cost_invoice_item_repository()) {
    throw new RuntimeException('KSeF module should cache cost invoice item repository instances.');
}
if ($module->cost_invoice_audit_repository() !== $module->cost_invoice_audit_repository()) {
    throw new RuntimeException('KSeF module should cache cost invoice audit repository instances.');
}
if ($module->cost_invoice_workflow_service() !== $module->cost_invoice_workflow_service()) {
    throw new RuntimeException('KSeF module should cache cost invoice workflow service instances.');
}
if ($module->ksef_import_service() !== $module->ksef_import_service()) {
    throw new RuntimeException('KSeF module should cache KSeF import service instances.');
}
if ($container->supplier_repository() !== $module->supplier_repository() || $container->cost_invoice_repository() !== $module->cost_invoice_repository() || $container->cost_invoice_item_repository() !== $module->cost_invoice_item_repository() || $container->cost_invoice_audit_repository() !== $module->cost_invoice_audit_repository()) {
    throw new RuntimeException('Container should delegate KSeF repositories to the KSeF module.');
}
if ($container->cost_invoice_workflow_service() !== $module->cost_invoice_workflow_service() || $container->ksef_import_service() !== $module->ksef_import_service()) {
    throw new RuntimeException('Container should delegate KSeF services to the KSeF module.');
}

$workflow = $container->cost_invoice_workflow_service();
if ($workflow->invoice_repository !== $module->cost_invoice_repository() || $workflow->audit_repository !== $module->cost_invoice_audit_repository() || $workflow->supplier_repository !== $module->supplier_repository() || $workflow->item_repository !== $module->cost_invoice_item_repository()) {
    throw new RuntimeException('Cost invoice workflow should use module-managed repositories.');
}
if ($workflow->project_repository !== $container->project_repository()) {
    throw new RuntimeException('Cost invoice workflow should use container-managed project repository.');
}

$ksef = $container->ksef_import_service();
if ($ksef->workflow_service !== $workflow || $ksef->invoice_repository !== $module->cost_invoice_repository() || $ksef->audit_repository !== $module->cost_invoice_audit_repository() || $ksef->supplier_repository !== $module->supplier_repository()) {
    throw new RuntimeException('KSeF import service should use module-managed invoice dependencies.');
}
if ($ksef->client_repository !== $container->client_repository()) {
    throw new RuntimeException('KSeF import service should use container-managed client repository.');
}

echo "Assertions: 13\n";
echo "KSeF module wiring test passed.\n";
