...
$fragments = [
    ...
    [$syncService, 'function test_connection()', 'Sync service should expose lightweight connection diagnostics endpoint.'],
    ...
    [$syncService, '/security/public-key-certificates', 'Sync service should call KSeF public key certificates endpoint.'],
    ...
    [$syncService, "'https://api.ksef.mf.gov.pl/v2'", 'Sync service should default to official KSeF API base URL (v2).'],
    ...
    [$syncService, "'x-continuation-token'", 'Sync service should support continuation token pagination header.'],
    ...
    [$syncService, '/auth/ksef-token', 'Sync service should call ksef-token auth endpoint.'],
    [$syncService, "'Nip', 'nip', 'onip'", 'Sync service should try contextIdentifier.type variants (starting with Nip) for compatibility.'],
    [$syncService, '/auth/', 'Sync service should poll auth operation status endpoint before redeeming token.'],
    [$syncService, '/auth/token/redeem', 'Sync service should redeem authentication token to API tokens.'],
    ...
    [$admin, "case 'ksef_api_test_connection':", 'Admin runtime should handle manual KSeF connection test action.'],
    [$admin, "case 'ksef_fetch_public_key':", 'Admin runtime should keep manual fetch of MF public key for diagnostics.'],
    [$admin, 'function handle_ksef_api_sync_now()', 'Admin runtime should expose manual KSeF sync handler.'],
    [$admin, 'function handle_ksef_api_test_connection()', 'Admin runtime should expose KSeF connection test handler.'],
    [$admin, 'function handle_ksef_fetch_public_key()', 'Admin runtime should expose public key fetch handler.'],
    [$admin, "ksef_ap_token", 'Admin runtime should persist AP token required for new auth flow.'],
    ...
    [$settingsTemplate, 'Klucz publiczny KSeF (MF) pobiera się automatycznie', 'Settings template should explain automatic public key retrieval.'],
    [$settingsTemplate, 'id="erp-omd-ksef-api-sync-now-form"', 'Settings template should expose manual KSeF sync form.'],
    [$settingsTemplate, 'id="erp-omd-ksef-api-test-connection-form"', 'Settings template should expose KSeF test-connection form.'],
    [$settingsTemplate, 'Test połączenia KSeF', 'Settings template should expose KSeF test-connection button.'],
