...
<body class="erp-omd-front-body">
    <main class="erp-omd-front-shell erp-omd-front-shell-dashboard">
        <section class="erp-omd-front-card erp-omd-front-card-wide">
            ...
            <div class="erp-omd-front-grid erp-omd-front-grid-manager erp-omd-front-grid-manager-full" data-manager-tab-pane="wpisy">
                <article class="erp-omd-front-panel">
                    <div class="erp-omd-front-section-heading">
                        ...
                        <table class="erp-omd-front-table" data-table-enhanced="1">
                            ...
                            <tbody>
                                <?php if ($manager_time_entries) : ?>
                                    <?php foreach ($manager_time_entries as $manager_time_entry) : ?>
                                        <tr>
                                            <td><?php echo esc_html((string) ($manager_time_entry['entry_date'] ?? '—')); ?></td>
                                            <td><?php echo esc_html((string) ($manager_time_entry['client_name'] ?? '—')); ?></td>
                                            <td><?php echo esc_html((string) ($manager_time_entry['project_name'] ?? '—')); ?></td>
                                            <td><?php echo esc_html((string) ($manager_time_entry['role_name'] ?? '—')); ?></td>
                                            <td><?php echo esc_html(number_format_i18n((float) ($manager_time_entry['hours'] ?? 0), 2)); ?></td>
                                            <td><?php echo esc_html([
                                                'submitted' => __('Oczekuje', 'erp-omd'),
                                                'approved' => __('Zaakceptowany', 'erp-omd'),
                                                'rejected' => __('Odrzucony', 'erp-omd'),
                                            ][(string) ($manager_time_entry['status'] ?? 'submitted')] ?? (string) ($manager_time_entry['status'] ?? 'submitted')); ?></td>
                                            <td><?php echo esc_html(wp_trim_words((string) ($manager_time_entry['description'] ?? ''), 18)); ?></td>
                                        </tr>
                                    <?php endforeach; ?>
                                <?php else : ?>
                                    <tr><td colspan="7"><?php esc_html_e('Brak wpisów czasu dla bieżącego managera.', 'erp-omd'); ?></td></tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
                    </div>
    ...
    (function () {
        var setupManagerTabs = function () {
            ...
            var allowedTabs = ['dodaj-wpis', 'wpisy', 'projekty', 'kosztorysy', 'akceptacje', 'wnioski'];
