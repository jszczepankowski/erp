...
- **Postęp bloku 1) OPTYMALIZACJA:**
  ...
  - **T2.1:** W TRAKCIE (wydzielenie wspólnych helperów front JS do `assets/js/front-shared.js` (`dedupeProjectRequestDateFields`, `setupCollapsibleSections`) + wyniesienie logiki tabs do dedykowanych assetów `assets/js/front-manager.js` i `assets/js/front-worker.js` + wyniesienie paginacji tabel worker do `front-worker.js`; kolejny krok: wyniesienie logiki tabel managera).
- **Następny krok wykonawczy (najbliższe wdrożenie):**
  1. kontynuacja **T2.1**: wyniesienie logiki tabel managera z inline `<script>` do `front-manager.js`,
