...
- **Postęp bloku 1) OPTYMALIZACJA:**
  ...
  - **T2.1:** W TRAKCIE (wydzielenie wspólnych helperów front JS do `assets/js/front-shared.js` (`dedupeProjectRequestDateFields`, `setupCollapsibleSections`) + wyniesienie logiki tabs do dedykowanych assetów `assets/js/front-manager.js` i `assets/js/front-worker.js`; kolejny krok: wynoszenie logiki tables do tych samych assetów).
- **Następny krok wykonawczy (najbliższe wdrożenie):**
  1. kontynuacja **T2.1**: wyniesienie logiki tables z inline `<script>` do `front-manager.js` i `front-worker.js`,
