...
- **Postęp bloku 1) OPTYMALIZACJA:**
  ...
  - **T2.1:** W TRAKCIE (wydzielenie wspólnych helperów front JS do `assets/js/front-shared.js` (`dedupeProjectRequestDateFields`, `setupCollapsibleSections`) + wyniesienie logiki tabs i paginacji tabel do dedykowanych assetów `assets/js/front-manager.js` i `assets/js/front-worker.js`; kolejny krok: domknięcie pozostałych inline helperów formularzy/filtrów).
- **Następny krok wykonawczy (najbliższe wdrożenie):**
  1. kontynuacja **T2.1**: wyniesienie pozostałych inline helperów formularzy/filtrów do `front-manager.js` i `front-worker.js`,
